...
def home(request):
    ...
    问卷首页首页视图
    :param request: 请求对象
    :return: 渲染视图: user_info: 用户信息; kind_info: 问卷信息;is_show_userinfo: 是否展示用户信息表单;user_info_has_entered: 是否已经录入表单;
    ...
    if kind_info.question_num > bank_info.total_question_num:  # 问卷出题数量是否小于题库总大小
...
def game(request):
    ...
    返回问卷题目信息的视图
    :param request: 请求对象
    :return: 渲染视图: user_info: 用户信息;kind_id: 问卷唯一标识;kind_name: 问卷名称;cop_finishat: 问卷结束时间;rule_text: 大赛规则;
    """
    uid = request.GET.get('uid', '')  # 获取uid
    kind_id = request.GET.get('kind_id', '')  # 获取kind_id
    try:  # 获取问卷信息
...
def result(request):
    ...
    问卷结果和排行榜的视图
    :param request: 请求对象
    :return: 渲染视图: qa_info: 答题记录数据;user_info: 用户信息数据;kind_info: 问卷信息数据;rank: 该用户当前问卷排名
    """
...
def rank(request):
    ...
    排行榜数据视图
    :param request: 请求对象
    :return: 渲染视图: user_info: 用户信息;kind_info: 问卷信息; rank: 所有问卷排名;
    """
...
def search(request):
    ...
    搜索查询视图
    :param request: 请求对象
    :return: 渲染视图: user_info: 用户信息;result:查询结果问卷信息集合;key: 查询结果的关键字,是根据问卷名称查询还是根据赞助商关键字查询的结果
    """
