...
class CompetitionKindInfo(CreateUpdateMixin):
    ...
    kind_id = ShortUUIDField(_(u'问卷id'), max_length=32, blank=True, null=True, help_text=u'问卷类别唯一标识', db_index=True)
    ...
    kind_type = models.IntegerField(_(u'问卷类型'), default=IT_ISSUE, choices=KIND_TYPES, help_text=u'问卷类型')
    kind_name = models.CharField(_(u'问卷名称'), max_length=32, blank=True, null=True, help_text=u'竞赛类别名称')
    ...
    # 周期相关
    cop_startat = models.DateTimeField(_(u'问卷开始时间'), default=timezone.now, help_text=_(u'问卷开始时间'))
    period_time = models.IntegerField(_(u'答题时间'), default=60, help_text=u'答题时间(min)')
    cop_finishat = models.DateTimeField(_(u'问卷结束时间'), blank=True, null=True, help_text=_(u'问卷结束时间'))

    # 参与相关
    total_partin_num = models.IntegerField(_(u'total_partin_num'), default=0, help_text=u'总参与人数')

    class Meta:
        verbose_name = _(u'问卷类别信息')
        verbose_name_plural = _(u'问卷类别信息')
...
class BankInfo(CreateUpdateMixin):
    ...
    kind_num = models.IntegerField(_(u'问卷使用次数'), default=0, help_text=u'问卷使用次数')
...
class ChoiceInfo(CreateUpdateMixin, MediaMixin):
    ...
    item5 = models.CharField(_(u'选项5'), max_length=255, blank=True, null=True, help_text=u'选项五')
...
class CompetitionQAInfo(CreateUpdateMixin):
    ...
    kind_id = models.CharField(_(u'问卷id'), max_length=32, blank=True, null=True, help_text=u'问卷类别唯一标识', db_index=True)
    ...
    class Meta:
        verbose_name = _(u'问卷问题记录')
        verbose_name_plural = _(u'问卷问题记录')
